...
def create_autocomplete_entry(parent, var: tk.StringVar, values_fn, width=18, on_enter=None, pick_transform=None):
    ...
    def on_tab(_e):
        if popup.visible():
            ...
            _hide_popup()
            return "break"
        # behave like Enter (optional action)
        if callable(on_enter):
            on_enter()
            return "break"
        return None
...
class InlineCellEditor:
    ...
    def start_edit(self, row_iid: str, col_id: str):
        ...
        if spec.get("type") == "combo":
            ...
            def on_down_key(_e):
                if not self.popup.visible():
                    refresh_popup(force_all=True, show_all=True)  # SHOW ALL
                    return "break"  # do not move on first open
            ...
            ent.insert(0, value)
            ent.focus()
            if col_id in ("price", "sale_price", "date", "voucher", "item_description"):
                try:
...
class SearchDialog(_SingletonIdleDialog):
    def __init__(self, app):
        ...
        add_field(0, 0, "Brand (מותג)", self.brand_var, values_fn=lambda: list_brands())
        add_field(0, 1, "Model (דגם)", self.model_var,
                  values_fn=lambda: (list_models_for_brand(self.brand_var.get().strip())
                                     if self.brand_var.get().strip() else list_models_all()))
        add_field(0, 2, "SKU (מק״ט)", self.sku_var,
        ...
        add_field(1, 3, "Seller (מוכר)", self.seller_var, values_fn=lambda: list_sellers())

        add_field(
            0, 3, "Type (שמש/ראייה)", self.kind_var,
            width=16,
        ...
        add_field(1, 0, "Year (שנה)", self.year_var, width=10, values_fn=lambda: [str(y) for y in list_years_from_months()])
        add_field(1, 1, "From month (מחודש)", self.month_from_var,
                  values_fn=lambda: month_values_for_selected_year(),
                  pick_transform=month_pick_transform)
        add_field(1, 2, "To month (עד חודש)", self.month_to_var,
                  values_fn=lambda: month_values_for_selected_year(),
                  pick_transform=month_pick_transform)

        add_field(2, 0, "Voucher (ואוצ'ר)", self.goal_voucher_var, width=18, values_fn=lambda: list_goal_vouchers())
        add_field(2, 1, "Description (תיאור מוצר)", self.goal_desc_var, width=28, values_fn=lambda: list_goal_descriptions())
        add_field(2, 2, "Lead from (מקור הגעה)", self.goal_lead_var, width=22, values_fn=lambda: list_goal_leads())

        btn_frame = ttk.Frame(top)
        btn_frame.grid(row=2, column=6, columnspan=4, sticky="w", padx=(10,0), pady=(0,6))
...
class YearlySummaryDialog(_SingletonIdleDialog):
    ...
    _CLR_REMAINING_NEG = "#C6EFCE"   # light red   – missed target
    _CLR_REMAINING_POS = "#FF9999"   # light green – hit target 
    ...
    _ROW_H = 26   # row height in pixels
    ...
    def _cell(self, parent, grid_row, col_idx, text, bg,
        ...
        lbl.grid(row=grid_row, column=col_idx, sticky="nsew", ipadx=2, ipady=3)
        if cursor:
    ...
    def _fill_data_row(self, parent, grid_row, col_widths,
        ...
        for ci, col in enumerate(all_cols):
            if col == "month":
                ...
                tgt    = month_data["target"]
                rem_bg = (self._CLR_REMAINING_NEG if rem < 0
                          else (self._CLR_REMAINING_POS if tgt > 0 else bg))
                self._cell(parent, grid_row, ci,
    ...
    def _fill_total_row(self, parent, grid_row, summary, all_cols, seller_cols):
        for ci, col in enumerate(all_cols):
            if col == "month":
                ...
                rem_bg = (self._CLR_REMAINING_NEG if total < 0
                          else self._CLR_REMAINING_POS)
                self._cell(parent, grid_row, ci,
...
class ManageMonthsDialog(_SingletonIdleDialog):
    """
    Unified month management window — shows all months from both sales and
    goals, and operates on both tables at once. Both toolbar buttons open
    this same class.
    """
    def __init__(self, app):
        super().__init__(app)
        ...
        # ---- Left: month list ----
        ttk.Label(frm, text="Months (חודשים):", font=("Segoe UI", 10, "bold")).grid(
            row=0, column=0, sticky="w", pady=(0, 4))
        self.lb = tk.Listbox(frm, height=14, width=28, font=("Segoe UI", 10))
        self.lb.grid(row=1, column=0, rowspan=10, sticky="ns", padx=(0, 16))
        self.lb.bind("<<ListboxSelect>>", self._on_select)

        # ---- Right: Add ----
        ttk.Label(frm, text="Add new month / הוסף חודש:").grid(
            row=1, column=1, sticky="w")
        self.add_e = ttk.Entry(frm, width=24)
        self.add_e.grid(row=2, column=1, sticky="we", pady=(2, 0))
        self.add_e.bind("<Return>", lambda e: self.add())
        ttk.Button(frm, text="➕ Add / הוסף", command=self.add).grid(
            row=3, column=1, sticky="we", pady=(4, 12))

        # ---- Right: Rename ----
        ttk.Label(frm, text="Rename selected / שנה שם:").grid(
            row=4, column=1, sticky="w")
        self.ren_e = ttk.Entry(frm, width=24)
        self.ren_e.grid(row=5, column=1, sticky="we", pady=(2, 0))
        self.ren_e.bind("<Return>", lambda e: self.rename())
        ttk.Button(frm, text="✏️ Rename / שנה", command=self.rename).grid(
            row=6, column=1, sticky="we", pady=(4, 12))

        # ---- Right: Delete ----
        ttk.Label(frm, text="Delete selected month / מחק חודש:").grid(
            row=7, column=1, sticky="w")
        ttk.Label(frm, text="(removes all sales & goals for that month)",
                  font=("Segoe UI", 8), foreground="gray").grid(
            row=8, column=1, sticky="w")
        ttk.Button(frm, text="❌ Delete Month / מחק חודש", command=self.delete).grid(
            row=9, column=1, sticky="we", pady=(4, 0))

        # ---- Close ----
        ttk.Button(frm, text="Close / סגור", command=self.destroy).grid(
            row=10, column=1, sticky="e", pady=(20, 0))

        self.reload()

    def _all_months(self):
        """Union of months from both sales and goals UI, sorted chronologically."""
        months = set(self.app.months_in_ui()) | set(self.app.goals_months_in_ui())
        def key_fn(m):
            k = month_to_key(m)
            return k if k else (9999, 99, m)
        return sorted(months, key=key_fn)

    def reload(self):
        sel_text = self.selected()
        self.lb.delete(0, tk.END)
...
class App(tk.Tk):
    ...
    def build_sales(self):
        ...
        if months:
            ...
            self.ensure_month_tab("ינואר 2026")
            self.reorder_month_tabs()
        ...
        # Ensure the current month exists as a tab (even if it has no sales yet)
    ...
    def build_goals(self):
        ...
        if months:
            ...
            self.ensure_goal_month_tab("ינואר 2026")
            self.reorder_goal_month_tabs()
    ...
    def refresh_all_goal_months(self):
        for m in list_months_from_db_sorted():
            self.ensure_goal_month_tab(m)
        # Also ensure goal-only months (months in goals table but not in sales)
    ...
    def _build_sales_tree(self, parent, month):
        ...
        col_specs = {
            ...
            "kind":  {"type":"combo", "restrict": False, "values_fn": lambda _row: ["Vision (ראיה)", "Sunglasses (שמש)"]},
            "price": {"type":"entry"},
            "seller": {"type":"combo", "restrict": False, "values_fn": lambda _row: list_sellers()},
