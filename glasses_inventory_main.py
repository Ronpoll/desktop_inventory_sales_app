...
class App(tk.Tk):
    ...
    # ------------------------------------------------------------------
    # Inventory per-brand selection state: {brand -> {"model": str, "sku": str}}
    # "" means "All" (no filter).  Display label stored separately.
    # ------------------------------------------------------------------
    def _inv_sel(self):
    ...
    def _inv_display_model(self, brand):
        """Label to show in the Model column for a brand row."""
    ...
    def _inv_display_sku(self, brand):
        """Label to show in the SKU column for a brand row."""
        sel = self._inv_sel().get(brand, {})
        s = sel.get("sku", "")
    ...
    def build_inventory(self):
        ...
        def _refresh_sku_cb(brand, preserve=True):
            ...
            if cur_sku and cur_sku in sku_cb_values:
        ...
        def _update_combos_for_brand(brand, preserve=True):
            sel = self._inv_sel().setdefault(brand, {"model": "", "sku": ""})
            models = list_models_for_brand(brand)
            real_models   = [m for m in models if not is_placeholder_model(m)]
            has_placeholder = any(is_placeholder_model(m) for m in models)
            model_cb_values = ["All"] + (["— (no model)"] if has_placeholder else []) + real_models
        ...
        def _compute_onhand(brand):
            """On-hand for brand given current model/sku selection."""
            sel = self._inv_sel().get(brand, {})
            model_sel = sel.get("model", "")
            sku_sel   = sel.get("sku",   "")
            with connect() as c:
                if model_sel and sku_sel:
                    ...
                    return inventory_on_hand(c, int(row[0])) if row else 0
                elif model_sel:
                    rs = c.execute(
                        "SELECT id FROM items WHERE brand=? AND model=?",
                        (brand, model_sel)
                    ).fetchall()
                    return sum(inventory_on_hand(c, int(r[0])) for r in rs)
                elif sku_sel:
                    rs = c.execute(
        ...
        def _sync_row(brand):
            """Update the tree row text and on-hand to match current selection."""
            iid = self._inv_brand_iid(brand)
            if not self.inv_tree.exists(iid):
                return
            self.inv_tree.set(iid, "model",  self._inv_display_model(brand))
            self.inv_tree.set(iid, "sku",    self._inv_display_sku(brand))
            self.inv_tree.set(iid, "onhand", _compute_onhand(brand))
        ...
        def _on_tree_select(event):
            sel = self.inv_tree.selection()
            if not sel or not sel[0].startswith("brand::"):
                self._inv_model_cb.place_forget()
                self._inv_sku_cb.place_forget()
                self._inv_cb_brand = None
                return
            brand = sel[0][len("brand::"):]
        ...
        vsb.configure(command=lambda *a: (self.inv_tree.yview(*a), _on_scroll()))
        self.inv_tree.bind("<MouseWheel>", lambda e: self.inv_tree.after_idle(_on_scroll))
        self.inv_tree.bind("<Button-4>",   lambda e: self.inv_tree.after_idle(_on_scroll))
        self.inv_tree.bind("<Button-5>",   lambda e: self.inv_tree.after_idle(_on_scroll))

        # Store closures needed by refresh_inventory
        self._inv_place_combos  = _place_combos
        self._inv_update_combos = _update_combos_for_brand
        self._inv_sync_row      = _sync_row
    ...
    def refresh_inventory(self):
        ...
        # Collect distinct brands and total on-hand for summary
        brand_total = {}
        with connect() as c:
            rows = c.execute(
            ...
            for item_id, brand, sku, model in rows:
                ...
                try:
                    ...
                    pass
                brand_total[brand] = brand_total.get(brand, 0) + onhand

        for brand in brand_total:
            iid = self._inv_brand_iid(brand)
            # Use saved selection labels (default "All") and recompute on-hand
            model_disp = self._inv_display_model(brand)
            sku_disp   = self._inv_display_sku(brand)
            sel = self._inv_sel().get(brand, {})
            model_sel  = sel.get("model", "")
            sku_sel    = sel.get("sku",   "")
            with connect() as c:
                if model_sel and sku_sel:
                    ...
                    display_onhand = brand_total[brand]
            self.inv_tree.insert("", "end", iid=iid,
                                 values=(brand, model_disp, sku_disp, display_onhand))
        ...
        # Restore selected brand and reposition comboboxes
